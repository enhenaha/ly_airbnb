...
const ScrollView = memo((props) => {
  ...
  function controlClickHandle(isRight) {
    ...
    totalDistanceRef.current = scrollContentRef.current.scrollWidth - scrollContentRef.current.clientWidth // 不使用之前保存的 "拉动的宽度"而是重新计算, 因为有些数据有点奇怪, 必须重新计算才能获取到正确的 "拉动的宽度"。也就是需要页面打开后等一会才能获取到正确的 "拉动的宽度", 因此点击按钮时在这里重新计算, 那么就一定能获取到正确的 '拉动的宽度'。
    // setShowRight(totalDistanceRef.current > newOffsetLeft) // 使用保存的 "拉动的宽度"
    setShowRight(totalDistanceRef.current > newOffsetLeft)
    setShowLeft(newOffsetLeft > 0)


    // 从上可知, useState这个hook不是每执行到一个 "修改数据的函数"后就立马重新调用当前组件函数, 而是等待一定的时机后一起重新调用当前函数的, 例如当在一个函数中存在多个可以使当前组件重新渲染的useState的 "修改数据的函数", 例如 setPosIndex、setShowRight、setShowLeft 这三个每一个函数执行都可以让当前函数重新被调用, 但是, 不是说一执行到setPosIndex函数后, 就里面重新调用当前函数, 那么这样后面的setShowRight、setShowLeft函数都无法执行的。react的机制是如果多个useState的 "修改数据的函数"在同一时间同时执行, 例如都在一个函数controlClickHandle中执行, 那么它就会等这个controlClickHandle函数中所有 "修改数据的函数"都执行完了才会重新渲染当前组件。至于这时候组件被渲染一次 还是 一次性渲染三次就不知道了。因此这里是 setPosIndex、setShowRight、setShowLeft这三个函数都执行完后, 当前组件才会重新渲染。
  }
